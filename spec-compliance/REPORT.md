...
| ----------- | --: | --------------------------------------------------------------------------------------------------------------------- |
| ✅ Conforme |  13 | AUTH-01, AUTH-02, AUTH-03, AUTH-04, AUTH-05, AUTH-06, AUTH-07, RN-AUTH-03, RN-AUTH-04, ROT-05, GPS-01, GPS-02, GPS-03 |
| ⚠️ Parcial  |   4 | RN-AUTH-02, ROT-07, GPS-06, EMB-01                                                                                    |
| ❌ Ausente  |   3 | RN-AUTH-01, RN-AUTH-05, GPS-04/GPS-05                                                                                 |

Conformidade plena em **13 de 20** (65%). As três ausências estão detalhadas abaixo;
a de GPS-04/05 é a que tem consequência operacional real.
...
| RN-AUTH-02 | 5 tentativas em 15 min → bloqueio progressivo      | ⚠️       | `users.service.ts:38,39,263`. Ver Lacuna 4                                                                                                                                       |
...
| RN-AUTH-05 | Logout com viagem em andamento exige confirmação   | ❌       | **Nenhuma ocorrência.** Ver Lacuna 2                                                                                                                                             |
...
| ROT-07 | Sequência de paradas e alunos | ⚠️       | `RouteStop`/`RouteStudent` no schema; ordenação por sentido em `apps/mobile/src/features/routes/stop-direction.ts`. Ver Lacuna 5                        |
...
| GPS-04 | Modo offline                  | ❌       | **Não implementado.** Ver Lacuna 3                                                                                                                      |
| GPS-05 | Reconexão (envio do buffer)   | ❌       | **Não implementado.** Ver Lacuna 3                                                                                                                      |
| GPS-06 | Precisão do GPS               | ⚠️       | `background-trip-location-task.ts:68` envia `precisaoMetros`; `use-my-location.ts:48` usa `Accuracy.Low`. Ver Lacuna 6                                  |
| EMB-01 | Checklist de embarque         | ⚠️       | `use-driver-trip.ts:145` (`useAddStudentEvent`); `inicio-screen.tsx:2036,2055` (`EMBARQUE`/`DESEMBARQUE`); backend `trips.service.ts:933`. Ver Lacuna 7 |
...
### Lacuna 2 — RN-AUTH-05: logout durante viagem não pede confirmação ❌
...
### Lacuna 3 — GPS-04 e GPS-05: não existe fila offline ❌
...
### Lacuna 5 — ROT-07: ordenação de paradas vive no cliente ⚠️
...
### Lacuna 7 — EMB-01: checklist existe como evento, não como checklist ⚠️
