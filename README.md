...
### Especificação Funcional Oficial (Partes 1–7)

Documento normativo de referência para desenvolvedores, designers, QA e futuros colaboradores — cada funcionalidade segue o padrão Objetivo/Descrição/Usuários envolvidos/Pré-requisitos/Fluxo principal/Fluxos alternativos/Regras de negócio/Permissões/Validações/Mensagens/Casos excepcionais/Critérios de aceite (BDD)/Melhorias futuras, com código estável de rastreabilidade (ex. `AUTH-01`, `ROT-05`, `CASO-13`).

| Parte | Códigos | Conteúdo | Status |
|---|---|---|---|
| [`docs/15-especificacao-funcional-autenticacao.md`](docs/15-especificacao-funcional-autenticacao.md) | `AUTH-*` | Cadastro, Login, Recuperação de senha, Primeiro acesso, Logout, Sessões, Troca de senha | ✅ |
| [`docs/16-especificacao-funcional-empresas-motoristas-veiculos.md`](docs/16-especificacao-funcional-empresas-motoristas-veiculos.md) | `EMP-*`, `DRV-*`, `VEI-*` | Empresas (cadastro, edição, plano, status, configurações), Motoristas (cadastro, CNH, EAR, facial, documentos, ativação, inativação, trocas), Veículos (cadastro, edição, capacidade, documentação, histórico) | ✅ |
| [`docs/17-especificacao-funcional-responsaveis-alunos-escolas.md`](docs/17-especificacao-funcional-responsaveis-alunos-escolas.md) | `RESP-*`, `STU-*`, `ESC-*` | Responsáveis (cadastro, vinculação automática, confirmação, segurança, alteração de dados), Alunos (cadastro, escola, turma, turno, responsáveis, pontos, status, histórico), Escolas (cadastro, INEP, turnos) | ✅ |
| [`docs/18-especificacao-funcional-rotas-gps-embarque-desembarque.md`](docs/18-especificacao-funcional-rotas-gps-embarque-desembarque.md) | `ROT-*`, `GPS-*`, `EMB-*`, `DESEMB-*` | Rotas (criação, edição, duplicação, dias da semana, substituições, sequência de alunos, otimização), GPS (início/fim de viagem, tempo real, offline, reconexão, precisão), Embarque (checklist, QR/NFC/facial futuros, lista manual), Desembarque (confirmação, histórico, notificações) | ✅ |
| [`docs/19-especificacao-funcional-notificacoes-dashboard-relatorios.md`](docs/19-especificacao-funcional-notificacoes-dashboard-relatorios.md) | `NOTIF-*`, `DASH-*`, `REL-*` | Notificações (push, WhatsApp, SMS, e-mail), Dashboard (indicadores, mapa, receita estimada, viagens, motoristas, veículos, alunos), Relatórios (motoristas, alunos, rotas, GPS, viagens, documentos) | ✅ |
