 ...
 *
 * Notificação ao responsável é automática — já acontece no backend
 * (`TripsService.notifyStudentEvent`, Communication Engine) a cada
 * `POST /trips/:id/student-events`, nenhum código novo aqui.
 *
 * "Ao iniciar a rota novamente aparecerá a localização das unidades de
 * ensino": consequência natural do modelo — a parada de embarque da
 * viagem de volta referencia a própria `School` (`RouteStop.schoolId`,
 * lat/lng fixos e sempre os mesmos). "Menos aqueles que não foram à
 * escola": para paradas de embarque numa escola, consulta
 * `useStudentsAttendanceToday` e não permite embarque de quem foi
 * marcado `AUSENTE` hoje em qualquer viagem da empresa.
 *
 * Frente 7 (prevenção de riscos, vazamentos e erros): `SectionErrorBoundary`
 * envolvendo a página inteira, `ErrorState` com "tentar de novo" (nunca
 * spinner infinito), botões desabilitados durante `isPending` (evita
 * disparo duplicado de embarque/início/fim).
 */
...
function ExecutarRotaContent(): JSX.Element {
