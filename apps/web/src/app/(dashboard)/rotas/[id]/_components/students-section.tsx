...
/**
 * Parte 3 do assistente "Criar rota" (Frente 4 do plano aprovado —
 * pedido do usuário: "adicionar alunos (quantos alunos quiserem)").
 * Fonte dos candidatos: contratos `ATIVO` da empresa
 * (`useContractsList`) que ainda não estão nesta rota — nunca uma
 * lista arbitrária de alunos, só quem já tem contrato ativo com ESTA
 * transportadora.
 */
 ...
 * pra não esconder por engano um candidato válido (ex. aluno turno
 * INTEGRAL serve pra qualquer rota).
 */
function AddStudentCandidateRow({
   ...
   * um contrato `NEGOCIADO` de verdade segue pela Authentique, sem PDF
   * de termo de ciência. Blob autenticado, nunca um `<a href>` cru — a
   * rota exige o Bearer token do usuário logado.
   */
